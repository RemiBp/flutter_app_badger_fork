...
class ClusterManager<T extends ClusterItem> {
  ClusterManager(this._items, this.updateMarkers,
      {Future<platform_maps.Marker> Function(Cluster<T>)? markerBuilder,
  ...
  final Future<platform_maps.Marker> Function(Cluster<T>) markerBuilder;
  ...
  final void Function(Set<platform_maps.Marker>) updateMarkers;
  ...
  void setMapId(int mapId, {bool withUpdate = true}) async {
    _mapId = mapId;
    _zoom = await platform_maps.GoogleMapsFlutterPlatform.instance.getZoomLevel(mapId: mapId);
  ...
  void _updateClusters() async {
    ...
    final Set<platform_maps.Marker> markers =
  ...
  void onCameraMove(platform_maps.CameraPosition position, {forceUpdate = false}) {
    _zoom = position.zoom;
    if (forceUpdate) {
      updateMap();
    }
  ...
  Future<List<Cluster<T>>> getMarkers() async {
    ...
    final platform_maps.LatLngBounds mapBounds = await platform_maps.GoogleMapsFlutterPlatform.instance
        .getVisibleRegion(mapId: _mapId!);

    late platform_maps.LatLngBounds inflatedBounds;
  ...
  platform_maps.LatLngBounds _inflateBounds(platform_maps.LatLngBounds bounds) {
    ...
    return platform_maps.LatLngBounds(
      southwest: platform_maps.LatLng(bounds.southwest.latitude - lat, wLng),
      northeast:
          platform_maps.LatLng(bounds.northeast.latitude + lat, lng != 0 ? eLng : _maxLng),
    );
  }
  ...
  static Future<platform_maps.Marker> Function(Cluster) get _basicMarkerBuilder =>
      (cluster) async {
        return platform_maps.Marker(
          markerId: platform_maps.MarkerId(cluster.getId()),
  ...
  static Future<platform_maps.BitmapDescriptor> _getBasicClusterBitmap(int size,
      {String? text}) async {
    ...
    return platform_maps.BitmapDescriptor.fromBytes(data.buffer.asUint8List());
  }
} 